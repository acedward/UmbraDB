...
describe("Rule B: one block height, one transaction — the only observable states are none of H and all of H", () => {
  ...
  it(
    ...
    async () => {
      ...
      const ids: string[] = [];
      const held: HeldKey[] = [];
      // Deliberately NOT "K": registration is idempotent by `(net, fingerprint)`, so registering
      // the corpus's "K" here would return the very monitor every other case in this file scans,
      // and advancing it would break them from a distance. This case also runs LAST for the same
      // reason — a crash suite whose cases can reach each other is a suite whose failures are
      // about the order they ran in.
      for (const keyId of ["Kprime", "Kthird"] as const) {
