...
describe("loadApiConfig", () => {
  it("defaults to loopback — the only thing standing between an unauthenticated API and the network", () => {
    ...
    expect(config.schema).toBe("shielded_monitor");
  });
  ...
  it("refuses a page cap above the store's own cap, at boot", () => {
    ...
    expect(() => loadApiConfig({ API_MAX_PAGE: String(MAX_ASSOCIATION_PAGE + 1) })).toThrowError(
      ApiConfigError,
    );
    expect(loadApiConfig({ API_MAX_PAGE: String(MAX_ASSOCIATION_PAGE) }).maxPage).toBe(
      MAX_ASSOCIATION_PAGE,
    );
  });

  it("clamps the default page to the configured cap rather than exceeding it", () => {
    expect(loadApiConfig({ API_MAX_PAGE: "10" }).defaultPage).toBe(10);
    expect(() => loadApiConfig({ API_MAX_PAGE: "10", API_DEFAULT_PAGE: "11" })).toThrowError(
      ApiConfigError,
    );
  });
  ...
  it.each([
    ...
    if (variable === "API_PORT" && value === "8787 ") {
      expect(loadApiConfig({ [variable]: value }).port).toBe(8787);
      return;
    }
    try {
      loadApiConfig({ [variable]: value });
  ...
  it("admits port 0 — the shared-host case where the kernel picks a free port", () => {
    expect(loadApiConfig({ API_PORT: "0" }).port).toBe(0);
  });
});
