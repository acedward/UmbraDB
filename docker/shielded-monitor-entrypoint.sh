...
#
# One image, six commands. `exec` so the process that ends up as PID 1 is the service itself and
...
case "$command" in
  node)        exec node /app/dist-cli/shielded-monitor/node-cli.js "$@" ;;
  ...
  *)
    ...
    echo "usage: <node|balancer|derive-key|storage|archive-sync|client> [args]" >&2
    exit 64
    ;;
esac
