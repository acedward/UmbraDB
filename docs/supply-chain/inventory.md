...
|---|---|---|---|---|---|---|---|
| `@midnight-ntwrk/ledger-v8` | `file:vendor/ledger-v8-syshash` → **8.1.0-syshash.6** | committed local artifact + 31 SHA-256 checksums | source commit recorded in `vendor/ledger-v8-syshash/PROVENANCE.md` (no out-of-tree patch since `.6` — the former root-export patch is in the fork tree) | `SHA256SUMS` (all shipped files) | Apache-2.0 | Node-only transaction hashing, semantic decode, replay, block close, committed ledger-state-root comparison, and the node-side DUST tree mirror (retain-all replay + collapsed-update cuts) | Upstream Ledger 8 exports; replace vendor only with equivalent verified release |
