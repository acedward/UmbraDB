...
### Requirement: contract ledger state is captured per touched contract at ingest time

The system SHALL, when contract-state capture is enabled, persist the ledger-serialized state of
every contract touched by a block's regular transactions, fetched from the node's
`midnight_contractState` at that block's own hash, in the same atomic write as the block's other
rows (`design.md` §6b — a capture of applied state, the one category not recoverable from archived
bytes; mirrors the reference indexer's per-action runtime-API capture, not replay).

#### Scenario: A deploy and a call each capture state at their own block

- **WHEN** a block contains a regular transaction whose intents carry a deploy, call, or
  maintenance action
- **THEN** a `contract_states` row SHALL exist for that (block, contract address) after ingest
- **AND** the state bytes SHALL be content-addressed, so an unchanged state across many blocks
  stores one blob

#### Scenario: The capture agrees with the indexer while the indexer exists

- **WHEN** the same (block, contract address) state is fetched from the indexer's
  `contractAction.state`
- **THEN** the archived state bytes SHALL be byte-identical to it

#### Scenario: A failed action's address does not abort the block

- **WHEN** `midnight_contractState` errors for a touched address (e.g. a deploy inside a failed
  fallible segment, so the contract never existed)
- **THEN** ingest SHALL skip that address and complete the block
- **AND** the absence of the row SHALL be the record of that outcome

### Requirement: the zswap state root is archived per block

The system SHALL, when contract-state capture is enabled, store the node-reported zswap state root
on each ingested block row, and SHALL represent pre-capture rows as NULL rather than backfilling a
guessed value.

#### Scenario: Root transitions occur exactly at zswap-affecting blocks

- **WHEN** consecutive blocks' stored roots are compared
- **THEN** the root SHALL change exactly at blocks that added commitments or nullifiers
- **AND** SHALL be identical across blocks with no zswap activity
